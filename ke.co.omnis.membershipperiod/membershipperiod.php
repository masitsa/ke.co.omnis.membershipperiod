...
function membershipperiod_civicrm_postProcess($formName, $form) 
{
	if ( is_a( $form, 'CRM_Member_Form_MembershipRenewal' ) ) {
        ...
        $renewal_date = $form->getVar( 'endDate' );
        $endDate = CRM_Core_DAO::getFieldValue('CRM_Member_DAO_Membership', $membershipID, 'end_date');
		
		$query = "
		INSERT IGNORE INTO `civicrm_omnupdatemembershipperiod`
		(`membership_id`, `start_date`, `end_date`) VALUES
		('".$membershipID."', '".$endDate."', '".$renewal_date."')
		";
